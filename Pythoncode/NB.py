...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  3 16:18:55 2019
...
class NBClassifier(object):
    ...
    def __init__(self, var_list , lamda=0, name = "NB"):
        ...
        self.x_v_y = np.zeros((self.feature_number,max(self.var_size_ls[:-1]), self.klass_size),dtype=np.double)    
            
        """
        Output
        """
        self.predict_prob = np.zeros((self.klass_size),dtype=np.double)  # array contains probability  
    ...
    def update(self, point):        
        ...
        self.klass_count[klass_index] = self.klass_count[klass_index] +1    # Add new (actual response)
        ...
        for i in range(self.feature_number):                                 # iterate over all predictors
            ...
            self.x_v_y[i][value_index][klass_index] = self.x_v_y[i][value_index][klass_index] +1 
    ...
    def log_conditional(self, klass_index, feature): # features here is an array
        ...
        for i in range(self.feature_number): 
            ...
            sum_condition += math.log((self.x_v_y[i][value_index][klass_index] + 1/self.var_size_ls[i])\
    ...
    def log_evidence(self, features):
        join_array = np.zeros((self.klass_size),dtype=np.double)
    ...
    def klassify(self, point):
        ...
        if self.counter ==0:
            for klass_id in range(self.klass_size):
                self.predict_prob[klass_id] = 1/self.klass_size  
